...
/* hacky malloc */
unsigned int alloc_data_s1 = 1024;
unsigned char alloc_data1[1024];
unsigned char *data_pointer1 = alloc_data1;
unsigned int cur_pos1 = 0; 
                                 ...
                                 unsigned char *output )
{
    ...
    *p++ = 0;
    ...
    EMSG0("%s [4]: ( mode == MBEDTLS_RSA_PUBLIC ) = %d\n", __func__, ( mode == MBEDTLS_RSA_PUBLIC ));
    *p++ = 0;
    memcpy( p, input, ilen );
                               ...
                               unsigned char *sig )
{
    ...
    if( mode == MBEDTLS_RSA_PRIVATE && ctx->padding != MBEDTLS_RSA_PKCS_V15 )
        return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
	EMSG1("pkcs1 signed[1], ret = %d, ctx->len = %#x\n", ret, ctx->len);
    olen = ctx->len;
    nb_pad = olen - 3;
    ...
    if( md_alg != MBEDTLS_MD_NONE )
    {
        ...
        if( md_info == NULL )
            return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
	EMSG1("pkcs1 signed[2], ret = %d\n", ret);

        if( mbedtls_oid_get_oid_by_md( md_alg, &oid, &oid_size ) != 0 )
            return( MBEDTLS_ERR_RSA_BAD_INPUT_DATA );
	EMSG1("pkcs1 signed[3], ret = %d\n", ret);
    ...
    *p++ = 0;
    ...
    EMSG1("pkcs1 signed[5], ret = %d, ctx->len = %#x\n", ret, ctx->len);
	...
	EMSG1("pkcs1 signed[7], ret = %d\n", ret);

    MBEDTLS_MPI_CHK( mbedtls_rsa_private( ctx, f_rng, p_rng, sig, sig_try ) );
    EMSG1("pkcs1 signed[7.1], ret = %d\n", ret);
    MBEDTLS_MPI_CHK( mbedtls_rsa_public( ctx, sig_try, verif ) );
    EMSG1("pkcs1 signed[7.2], ret = %d\n", ret);
